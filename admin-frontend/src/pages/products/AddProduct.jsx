...
const ProductAdmin = () => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      setIsLoading(true);
      try {
        const response = await productService.getProductUnits();
  ...
  // Add variant with all fields from ProductVariant struct
  const addVariant = () => {
    setProductData((prev) => ({
      ...prev,
      variants: [
        ...prev.variants,
        {
          ...
          size: "",
          color: "",
          weight: "", // Added from ProductVariant struct
        },
      ],
    }));
  };
  ...
  // Validation aligned with ProductVariant struct
  const validateForm = () => {
    const newErrors = {};
    ...
    productData.variants.forEach((variant, index) => {
      if (!variant.name.trim()) newErrors[`variant_${index}_name`] = `Variant ${index + 1}: Name is required`;
      if (!variant.price || Number(variant.price) <= 0) newErrors[`variant_${index}_price`] = `Variant ${index + 1}: Valid price is required`;
      // if (!variant.sku.trim()) newErrors[`variant_${index}_sku`] = `Variant ${index + 1}: SKU is required`;
      if (variant.stock && Number(variant.stock) < 0) newErrors[`variant_${index}_stock`] = `Variant ${index + 1}: Stock cannot be negative`;
      if (variant.quantity_value && Number(variant.quantity_value) < 0) newErrors[`variant_${index}_quantity_value`] = `Variant ${index + 1}: Quantity value cannot be negative`;
    });
  ...
  const normalizeProductData = (data) => {
    return {
      ...
      uom_id: data.uom_id ? Number(data.uom_id) : 0,
      quantity_value: data.quantity_value ? Number(data.quantity_value) : 0,
      variants: data.variants.map((variant) => ({
        ...variant,
        price: variant.price ? Number(variant.price) : 0,
        stock: variant.stock ? Number(variant.stock) : 0,
        uom_id: variant.uom_id ? Number(variant.uom_id) : 0,
        quantity_value: variant.quantity_value ? Number(variant.quantity_value) : 0,
        size: variant.size || '',
        color: variant.color || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Normalized productData:", JSON.stringify(normalizedData, null, 2));

      const response = await productService.createProduct(normalizedData);
    ...
    <div className="bg-gray-50 min-h-screen pb-10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
            ...
            {activeTab === "pricing" && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {productData.price && productData.discount > 0 && (
                    <div className="col-span-2">
                      <div className="bg-green-50 p-3 rounded border border-green-100">
                        <p className="text-green-800">
                          Sale price: <strong>₹{(productData.price * (1 - productData.discount / 100)).toFixed(2)}</strong> (You save: ₹{(productData.price * productData.discount / 100).toFixed(2)})
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <div className="space-y-6">
                ...
                {productData.variants.length === 0 ? (
                  <div className="bg-gray-50 border border-dashed border-gray-300 rounded-lg p-6 text-center">
                    <p className="text-gray-500">No variants added yet. Add variants for products with different sizes, colors, etc.</p>
                    <button
                      type="button"
                      onClick={addVariant}
                        ...
                        <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            {errors[`variant_${index}_stock`] && (
                              <p className="mt-1 text-sm text-red-500">{errors[`variant_${index}_stock`]}</p>
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              step="0.01"
                              className={`w-full p-2 border rounded ${
                                errors[`variant_${index}_quantity_value`] ? "border-red-500" : "border-gray-300"
                              }`}
                            />
                            {errors[`variant_${index}_quantity_value`] && (
                              <p className="mt-1 text-sm text-red-500">{errors[`variant_${index}_quantity_value`]}</p>
                            )}
                          </div>
            ...
            {activeTab === "attributes" && (
              <div className="space-y-6">
                ...
                {productData.attributes.length === 0 ? (
                  <div className="bg-gray-50 border border-dashed border-gray-300 rounded-lg p-6 text-center">
                    <p className="text-gray-500">No attributes added yet. Add attributes like Material, Weight, Dimensions, etc.</p>
                    <button
                      type="button"
                      onClick={addAttribute}
                      className="mt-3 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <button
            onClick={handleSubmit}
            className="px-6 py-2 bg-blue-600 border border-transparent rounded-md text-white font-medium hover:bg-blue-700 focus:outline-none"
            disabled={isLoading}
          >
